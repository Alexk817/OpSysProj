...
void SIO_addArived(std::vector<Process> &processes, std::vector<Process *> &ready_queue, int &curr_time, char *buff2)
{
    for (unsigned int i = 0; i < processes.size(); i++)
    {
        ...
        if (processes[i].arrival == curr_time)
        {
            ...
            std::sort(ready_queue.begin(), ready_queue.end(), SIO_compareTimeleft());

            //if it just arrived print this message
            if (processes[i].burst_num == 0)
            {
                sprintf(buff2, "%d", processes[i].SIO_tau);
...
void SIO_popQueifPossible(std::vector<Process *> &ready_queue, Process *&curr_process, int &curr_time, std::vector<Process> &processes, int &context_time, char *buff, char *buff2)
{
    if (ready_queue.size())
    {
        ...
        for (int j = 0; j < context_time / 2; j++)
        {
            ...
            if (j == context_time / 2 - 1)
            {
                ...
                sprintf(buff2, "%d", (*curr_process).SIO_tau);
            ...
            SIO_addArived(processes, ready_queue, curr_time, buff2);
        }
    }
}
...
std::vector<double> SIO(std::vector<Process> processes, int context_time)
{
    ...
    printEvent(curr_time, "Simulator started for SIO", ready_queue);
    ...
    while (active_processes.size())
    {
        ...
        {
            SIO_addArived(processes, ready_queue, curr_time, buff2);
            SIO_popQueifPossible(ready_queue, curr_process, curr_time, processes, context_time, buff, buff2);
        }
        else
        {
    ...
    printEvent(curr_time - 1, "Simulator ended for SIO", ready_queue);
    free(buff);
    free(buff2);
