...
void SJF_addArived(std::vector<Process> &processes, std::vector<Process *> &ready_queue, int &curr_time, char *buff2)
{
...
void SJF_popQueifPossible(std::vector<Process *> &ready_queue, Process *&curr_process, int &curr_time, std::vector<Process> &processes, int &context_time, char *buff, char *buff2)
{
    if (ready_queue.size())
    {
        ...
        for (int j = 0; j < context_time / 2; j++)
        {
            ...
            SJF_addArived(processes, ready_queue, curr_time, buff2);
        }
    }
}
...
std::vector<double> SJF(std::vector<Process> processes, int context_time)
{
    ...
    printEvent(curr_time, "Simulator started for SJF", ready_queue);
    ...
    while (active_processes.size())
    {
        ...
        {
            SJF_addArived(processes, ready_queue, curr_time, buff2);
            SJF_popQueifPossible(ready_queue, curr_process, curr_time, processes, context_time, buff, buff2);
        }
        else
        {
    ...
    printEvent(curr_time - 1, "Simulator ended for SJF", ready_queue);
    free(buff);
    free(buff2);
